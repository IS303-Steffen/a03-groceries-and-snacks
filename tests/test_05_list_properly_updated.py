max_score = 15  # Pulled by yml_generator.py to assign a score to this test.
...
def test_05_list_properly_updated(current_test_name, input_test_cases):
    ...
    try:
        ...
        for input_test_case in input_test_cases[0:2]:
            ...
            expected_lists_string = expected_lists_string.strip()
            ...
            # Find all variables in student's code that are of type list
            student_lists = {name: value for name, value in student_variables.items() if isinstance(value, list)}

            case_failed_messages = []

            # Assert that there is at least one list
            if not student_lists:
            ...
            normalized_student_list = []
            # Iterate over all lists found in the student's code
            for list_name, student_list in student_lists.items():
                list_name = list_name.rpartition(".")[-1]
                ...
                # Check if student list matches any of the expected list
                for normalized_expected_list in normalized_expected_lists:
                    if 'pasta' in normalized_student_list or input_test_case.get('id_input_test_case') == 1:
                        ...
                        break  # Stop once we find a matching list
                if list_match_found:
                    break
            student_lists_string = student_lists_string.strip()
            ...
            if not list_match_found:
                formatted = format_error_message(
                    custom_message=(
                        ...
                        f"matched the expected value for this input test case.\n\n"
